...
| **`checklist`** | builds nothing; checks the code against twelve fixed rules | *does it pass, rule by rule?* |
...
lenses hunt for whatever is wrong, then argue. `checklist` **closes** it — twelve rules fixed in
...
`checklist` closes it — **twelve rules, known in advance, each ending in PASS or FAIL.**
...
- **A rule you didn't check is `NOT CHECKED`, never PASS.** A short checklist is a checklist that was
  not run, so all twelve rows print every time, including the clean ones.
